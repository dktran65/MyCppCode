...
kwbuildproject --url http://localhost:8080/%projectName% -o kwtables kwinject.out --force --replace-path %CD%=%projectName%

REM *** 4. we load our result with kwadmin and load to the Klocwork Server
kwadmin --url http://localhost:8080 load %projectName%  kwtables